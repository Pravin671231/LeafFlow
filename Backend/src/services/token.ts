...
export function signBuyerAccessToken(payload: BuyerTokenPayload): string {
  return jwt.sign(payload, JWT_PRIVATE_KEY, { algorithm: "RS256", expiresIn: JWT_EXPIRES_IN });
}

export function verifyBuyerAccessToken(token: string): BuyerTokenPayload {
  const decoded = jwt.verify(token, JWT_PUBLIC_KEY, { algorithms: ["RS256"] });
  return decoded as BuyerTokenPayload;
}

export async function createBuyerRefreshToken(userId: Types.ObjectId): Promise<string> {
  const raw = randomBytes(32).toString("hex");
  const selector = raw.slice(0, 16);
  const tokenHash = await bcrypt.hash(raw, BCRYPT_ROUNDS_OTP);
  await RefreshToken.create({
...
export async function validateBuyerRefreshToken(
  raw: string
): Promise<{ userId: Types.ObjectId; tokenHash: string }> {
  const selector = raw.slice(0, 16);
  const record = await RefreshToken.findOne({
    selector,
    role: "buyer",
  ...
  const match = await bcrypt.compare(raw, record.tokenHash);
  if (!match) throw new Error("Invalid refresh token");
  if (!record.userId) throw new Error("Token has no userId");
  return { userId: record.userId as Types.ObjectId, tokenHash: record.tokenHash };
}
